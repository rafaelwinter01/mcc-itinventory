...
type ValidationMaps = {
	status: Record<string, NamedOption[] | null>
	devicetype: Record<string, NamedOption[] | null>
	location: Record<string, NamedOption[] | null>
	billedto: Record<string, NamedOption[] | null>
	costto: Record<string, NamedOption[] | null>
	user: Record<string, ValueOption[] | null>
	makemodel: Record<string, ValueOption[] | null>
}
...
type SearchFieldType = "status" | "device_type" | "location" | "department" | "user" | "make_model"
...
export default function DeviceImportPage() {
	...
	const [searchModalType, setSearchModalType] =
		React.useState<SearchFieldType>("status")
	...
	const handleUpload = React.useCallback(async () => {
		...
		try {
			...
			const response = await fetch("/api/device/import/upload", {
				method: "POST",
				body: formData,
			})
	...
	const deviceTypeCounts = React.useMemo(() => {
		...
		return (
			(validationPayload as ValidationPayload).found?.deviceTypeCounts ??
			null
		)
	}, [validationPayload])
	...
	const gridConfigs = React.useMemo(
		() =>
			[
				...
				{ key: "billedto", title: "Billed To" },
				{ key: "costto", title: "Cost To" },
				{ key: "user", title: "User" },
	...
	const handleProcessImport = React.useCallback(async () => {
		...
		setProcessErrors([])
		try {
			const response = await fetch("/api/device/import/process", {
				method: "POST",
				headers: {
			...
			const payload = await response.json().catch(() => ({}))
			console.log("Process import response:", payload)
	...
	const deviceTypeReport = React.useMemo(() => {
		if (!deviceTypeCounts || !validationMaps) {
			return null
		}

		const result: Record<string, number> = {}
		Object.entries(deviceTypeCounts).forEach(([csvValue, count]) => {
			const selectionKey = `devicetype::${csvValue}`
			const selectedId = validationSelections[selectionKey]
			const mappedOptions = validationMaps.devicetype[csvValue] ?? []
			...
			const selectedMatch = selectedId
				...
				: undefined
			const label =
				selectedMatch?.name ?? addedOption?.label ?? csvValue

			result[label] = (result[label] ?? 0) + count
		})

		return result
	}, [addedOptions, deviceTypeCounts, validationMaps, validationSelections])
		...
		<section className="space-y-6">
			<div className="space-y-2">
				...
				<p className="text-muted-foreground">
					Follow the steps to import devices from a CSV spreadsheet.
				</p>
			</div>
					...
					<TabsContent value="download" className="space-y-6">
						...
						<div className="flex min-h-65 flex-col items-center justify-center gap-4 rounded-xl border border-dashed p-6 text-center">
							...
							<Button asChild size="lg" className="h-12 px-8 text-base">
								<a href="/api/device/import/template" download>
					...
					<TabsContent value="import" className="space-y-6">
						<div className="space-y-2">
							<h2 className="text-xl font-semibold">
								The following devices will be processed.
							</h2>
							<p className="text-muted-foreground">
							...
							<div className="mt-4">
								{deviceTypeReport ? (
									<div className="overflow-hidden rounded-lg border">
										...
										<div className="divide-y">
											{Object.entries(deviceTypeReport).length === 0 ? (
												<div className="px-4 py-3 text-sm text-muted-foreground">
													No device types found.
												</div>
											) : (
												Object.entries(deviceTypeReport).map(
													([label, count]) => (
														<div
															key={label}
