...
export function ExportDevicesModal({
	...
	const moveToAvailable = () => {
		...
		setChosenFields((prev) =>
			prev.filter((field) => !selectedChosen.has(field))
		)
		setSelectedChosen(new Set())
	}
	...
	const handleApply = async () => {
		...
		if (response.ok) {
			...
			const filename = filenameMatch?.[1]?.replace(/"/g, "") || "devices-export.csv"
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-4xl">
				<DialogHeader>
					<DialogTitle>Export Devices to CSV</DialogTitle>
				</DialogHeader>
